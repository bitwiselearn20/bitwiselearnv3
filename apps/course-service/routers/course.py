...
from shared.schemas.course import (
    ...
    AddEnrollmentRequest, AddCodingProblemsRequest,
)
...
from shared.models.course_section_problem import CourseSectionProblem
...
from shared.enums import CourseStatus, UserType, ProblemStatus
...
# ========== SECTION CODING PROBLEMS (max 5 per submodule) ==========

MAX_SECTION_PROBLEMS = 5


@router.post("/add-coding-problems-to-section/{section_id}")
async def add_coding_problems_to_section(
    section_id: str, body: AddCodingProblemsRequest, current_user: dict = Depends(admin_only)
):
    """Attach DSA coding problems to a section. A section holds at most 5."""
    section = await CourseSection.get(PydanticObjectId(section_id))
    if not section:
        return api_response(404, "Section not found", error="Not found")

    existing = await CourseSectionProblem.find(
        CourseSectionProblem.section_id == section.id
    ).to_list()
    existing_ids = {str(e.problem_id) for e in existing}

    # de-dup requested ids, drop ones already attached
    requested = []
    for pid in body.problem_ids:
        if pid not in existing_ids and pid not in requested:
            requested.append(pid)

    if len(existing) + len(requested) > MAX_SECTION_PROBLEMS:
        return api_response(
            400,
            f"A section can have at most {MAX_SECTION_PROBLEMS} coding problems "
    ...
    for pid in requested:
        try:
            ...
            return api_response(400, f"Invalid problem id: {pid}", error="Invalid id")
        problem = await Problem.get(oid)
        if not problem:
            return api_response(404, f"Problem not found: {pid}", error="Not found")
        link = CourseSectionProblem(section_id=section.id, problem_id=oid)
        await link.insert()
        added.append({"id": str(problem.id), "name": problem.name, "difficulty": problem.difficulty})

    return api_response(201, "Coding problems added to section", data={
        "added": added,
        "total_attached": len(existing) + len(added),
        "remaining_slots": MAX_SECTION_PROBLEMS - (len(existing) + len(added)),
    })


@router.get("/get-section-coding-problems/{section_id}")
async def get_section_coding_problems(section_id: str, current_user: dict = Depends(get_current_user)):
    """Admin/teacher view: coding problems attached to a section."""
    links = await CourseSectionProblem.find(
        CourseSectionProblem.section_id == PydanticObjectId(section_id)
    ).to_list()
    problems = []
    for link in links:
        problem = await Problem.get(link.problem_id)
        if problem:
            problems.append({
                "id": str(problem.id), "name": problem.name,
                "difficulty": problem.difficulty, "published": problem.published,
            })
    return api_response(200, "Section coding problems", data={
        "problems": problems,
        "count": len(problems),
        "max": MAX_SECTION_PROBLEMS,
    })


@router.delete("/remove-coding-problem-from-section/{section_id}/{problem_id}")
async def remove_coding_problem_from_section(
    section_id: str, problem_id: str, current_user: dict = Depends(admin_only)
):
    link = await CourseSectionProblem.find_one(
        CourseSectionProblem.section_id == PydanticObjectId(section_id),
        CourseSectionProblem.problem_id == PydanticObjectId(problem_id),
    )
    if not link:
        return api_response(404, "Problem not attached to this section", error="Not found")
    await link.delete()
    return api_response(200, "Coding problem removed from section")


@router.get("/get-student-section-coding-problems/{section_id}")
async def get_student_section_coding_problems(
    section_id: str, current_user: dict = Depends(require_roles(UserType.STUDENT))
):
    """Student view: section coding problems with completed / not-completed status.
    A problem is 'completed' when the student has a SUCCESS submission for it."""
    student_id = PydanticObjectId(current_user["id"])
    links = await CourseSectionProblem.find(
        CourseSectionProblem.section_id == PydanticObjectId(section_id)
    ).to_list()
    ...
    for link in links:
        ...
        if not problem:
            continue
        solved = await ProblemSubmission.find_one(
            ProblemSubmission.student_id == student_id,
            ProblemSubmission.problem_id == link.problem_id,
            ProblemSubmission.status == ProblemStatus.SUCCESS,
        )
        problems.append({
            "id": str(problem.id), "name": problem.name,
            "difficulty": problem.difficulty, "completed": solved is not None,
        })
