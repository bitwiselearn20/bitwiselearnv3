...
def _normalize_lang(language: str) -> str:
    ...
    aliases = {"PY": "PYTHON", "JS": "JAVASCRIPT", "NODE": "JAVASCRIPT", "C++": "CPP", "CXX": "CPP"}
    return aliases.get(normalized, normalized)


def _piston_failed(result: dict) -> bool:
    """Detect when the execution server didn't actually run the code (e.g. the
    sandbox 'clone failed' error seen when Piston runs under x86 emulation)."""
    if not isinstance(result, dict):
        return True
    if result.get("error"):
        return True
    run = result.get("run") or {}
    stderr = (run.get("stderr") or "") + (run.get("message") or "")
    if "clone failed" in stderr or "proxy" in stderr.lower():
        return True
    if run.get("status") == "XX":
        return True
    return False


def _run_locally(language: str, code: str, stdin: str) -> dict | None:
    """Dev fallback: run code on the host when Piston is unavailable/broken.
    Supports Python and JavaScript (node). Returns a Piston-shaped dict, or None
    if the language/interpreter isn't available locally. NOT sandboxed — dev only."""
    lang = _normalize_lang(language)
    if lang == "PYTHON":
        cmd, suffix = [sys.executable or "python3"], ".py"
    ...
    try:
        ...
        tmp.close()
        proc = subprocess.run(
            cmd + [tmp.name], input=stdin, capture_output=True, text=True, timeout=10
        )
        return {
            "run": {
                "stdout": proc.stdout,
        ...
        return {"run": {"stdout": "", "stderr": "Time limit exceeded", "code": 1}, "version": "local-fallback"}
    except Exception as exc:
        return {"run": {"stdout": "", "stderr": f"Local execution error: {exc}", "code": 1}, "version": "local-fallback"}
    finally:
        try:
            os.unlink(tmp.name)
        except OSError:
...
async def execute_code(language: str, code: str, stdin: str = "") -> dict:
    """Run code via Piston; if Piston can't execute (unreachable or sandbox
    failure under emulation), fall back to local execution for Python/JS."""
    result = await _execute_via_piston(language, code, stdin)
    if _piston_failed(result):
        local = _run_locally(language, code, stdin)
        if local is not None:
            return local
    return result


async def _execute_via_piston(language: str, code: str, stdin: str = "") -> dict:
