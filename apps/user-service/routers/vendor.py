...
from shared.schemas.vendor import CreateVendorRequest, UpdateVendorRequest
...
async def get_all_vendors(current_user: dict = Depends(_read_roles)):
    ...
    data = [{
        ...
        "website_link": v.website_link, "display_password": v.display_password,
...
async def get_vendor_by_id(id: str, current_user: dict = Depends(get_current_user)):
    ...
    return api_response(200, "Vendor fetched", data={
        ...
        "display_password": vendor.display_password,
...
async def regenerate_vendor_password(id: str, current_user: dict = Depends(_delete_roles)):
    vendor = await Vendor.get(PydanticObjectId(id))
    if not vendor:
        return api_response(404, "Vendor not found", error="Not found")
    raw_password = secrets.token_urlsafe(10)
    vendor.login_password = hash_password(raw_password)
    vendor.display_password = raw_password
    vendor.updated_at = datetime.now(timezone.utc)
    await vendor.save()
    return api_response(200, "Password regenerated", data={
        "id": str(vendor.id), "email": vendor.email, "password": raw_password,
    })
