...
from shared.schemas.teacher import CreateTeacherRequest, UpdateTeacherRequest
...
async def get_all_teachers(current_user: dict = Depends(get_current_user)):
    ...
    data = [{
        ...
        "batch_id": str(t.batch_id), "display_password": t.display_password,
...
async def get_teacher_by_id(id: str, current_user: dict = Depends(get_current_user)):
    ...
    return api_response(200, "Teacher fetched", data={
        ...
        "display_password": teacher.display_password,
...
async def regenerate_teacher_password(id: str, current_user: dict = Depends(not_student)):
    teacher = await Teacher.get(PydanticObjectId(id))
    if not teacher:
        return api_response(404, "Teacher not found", error="Not found")
    raw_password = secrets.token_urlsafe(10)
    teacher.login_password = hash_password(raw_password)
    teacher.display_password = raw_password
    teacher.updated_at = datetime.now(timezone.utc)
    await teacher.save()
    return api_response(200, "Password regenerated", data={
        "id": str(teacher.id), "email": teacher.email, "password": raw_password,
    })
