...
async def forgot_password(body: ForgotPasswordRequest):
    ...
    data = {"email": email}
    if sent:
        return api_response(200, "OTP sent to email", data=data)

    # Dev fallback: SMTP isn't configured (or send failed), so the OTP can't be
    # emailed. Return it in the response so the flow is testable. This is gated
    # on EMAIL_USER/EMAIL_PASS being empty — set them to disable this behaviour.
    data["otp"] = otp
    data["dev_note"] = "Email not configured — OTP returned for development only."
    return api_response(200, "OTP generated (email not configured)", data=data)
...
async def verify_forgot_password(body: VerifyOtpRequest, response: Response):
    ...
    # Also return the token in the body: the frontend proxy reads it from the
    # response data and sets the reset_token cookie on its own origin (the
    # backend cookie can't round-trip across the gateway/proxy origin).
