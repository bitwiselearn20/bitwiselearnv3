...
import { getSectionProblems } from "@/api/courses/section/get-section-problems";
import { addSectionProblems } from "@/api/courses/section/add-section-problems";
import { removeSectionProblem } from "@/api/courses/section/remove-section-problem";
...
// ------------------------ Browse Problems Modal --------------------
interface BrowseProblemsModalProps {
  open: boolean;
  onClose: () => void;
  sectionId: string;
  attachedIds: string[];
...
const BrowseProblemsModal = ({
  ...
  useEffect(() => {
    ...
    setQuery("");
    setLoading(true);
    getAllProblemData(setAllProblems, true).finally(() => setLoading(false));
  }, [open]);
  ...
  const toggle = (id: string) => {
    setSelected((prev) => {
      ...
      if (prev.length >= remainingSlots) {
        toast.error(`You can add only ${remainingSlots} more problem(s)`);
        return prev;
      }
      return [...prev, id];
    });
  ...
  const submit = async () => {
    ...
    setSaving(true);
    const toastId = toast.loading("Adding problems...");
    try {
      await addSectionProblems(sectionId, selected);
      toast.success("Problems added to section", { id: toastId });
      onAdded();
      onClose();
    } catch (error: any) {
      toast.error(
        error?.response?.data?.message || "Failed to add problems",
        { id: toastId },
      );
    } finally {
      setSaving(false);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div
        className={`w-full max-w-2xl max-h-[80vh] flex flex-col rounded-xl ${Colors.border.defaultThick} ${Colors.background.secondary} p-5`}
      >
        <div className="flex items-center justify-between">
          ...
          <span className={`text-xs ${Colors.text.secondary}`}>
            {selected.length}/{remainingSlots} selected · {remainingSlots} slot(s)
            left
          </span>
        </div>
        ...
        <input
          ...
          className={`mt-3 w-full rounded-md px-3 py-2 text-sm ${Colors.border.defaultThin} ${Colors.background.primary} ${Colors.text.primary} outline-none`}
        />

        <div className="mt-3 flex-1 overflow-y-auto space-y-2 pr-1">
          {loading ? (
            <p className={`text-sm ${Colors.text.secondary}`}>Loading problems...</p>
          ) : filtered.length === 0 ? (
            <p className={`text-sm ${Colors.text.secondary}`}>No problems found</p>
          ) : (
            filtered.map((p) => {
              ...
              return (
                <label
                  key={p.id}
                  className={`flex items-center gap-3 rounded-lg px-4 py-2.5 ${Colors.border.defaultThin} ${Colors.background.primary} ${
                    isAttached ? "opacity-40" : "cursor-pointer"
                  }`}
                >
                  <input
                    type="checkbox"
                  ...
                  <span className={`text-xs ${Colors.text.secondary}`}>
                    {isAttached ? "already added" : p.difficulty}
                  </span>
                </label>
              );
            })
        ...
        <div className="mt-4 flex justify-end gap-3">
          <button
            onClick={onClose}
            className={`rounded-md px-4 py-1.5 text-xs ${Colors.border.defaultThin} ${Colors.text.secondary} cursor-pointer`}
          >
            Cancel
          </button>
          <button
            ...
            className={`rounded-md px-4 py-1.5 text-xs ${Colors.background.special} ${Colors.text.primary} cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed`}
          >
            {saving ? "Adding..." : `Add ${selected.length || ""} problem(s)`}
          </button>
        </div>
      </div>
    </div>
...
const AddSectionV2 = ({
  ...
  const [activeTab, setActiveTab] = useState<"TOPIC" | "ASSIGNMENT" | "PROBLEM">(
    "TOPIC",
  );
  const [isEditAssignmentOpen, setIsEditAssignmentOpen] = useState(false);

  // ---- coding problems attached to this section ----
  const [sectionProblems, setSectionProblems] = useState<any[]>([]);
  const [problemsLoading, setProblemsLoading] = useState(false);
  const [problemsRefetchKey, setProblemsRefetchKey] = useState(0);
  const [isBrowseProblemsOpen, setIsBrowseProblemsOpen] = useState(false);
  const MAX_SECTION_PROBLEMS = 5;
  ...
  // fetch coding problems attached to this section
  useEffect(() => {
    ...
    const fetchProblems = async () => {
      try {
        ...
        const res = await getSectionProblems(sectionId);
        setSectionProblems(res.data?.problems || []);
      } catch (error) {
    ...
    fetchProblems();
  }, [activeTab, sectionId, problemsRefetchKey]);

  const handleRemoveSectionProblem = async (problemId: string) => {
    const toastId = toast.loading("Removing problem...");
    try {
      await removeSectionProblem(sectionId, problemId);
      toast.success("Problem removed", { id: toastId });
      setProblemsRefetchKey((prev) => prev + 1);
    } catch (error) {
      toast.error("Failed to remove problem", { id: toastId });
    }
  };
  ...
  return (
    ...
    "
      ...
      {activeTab === "PROBLEM" && (
        <div className="mt-4 space-y-3">
          <p className={`text-xs ${Colors.text.secondary}`}>
            {sectionProblems.length}/{MAX_SECTION_PROBLEMS} coding problems added
          </p>
          {problemsLoading ? (
            <p className={`text-sm ${Colors.text.secondary}`}>
              Loading problems...
            ...
            rounded-lg ${Colors.border.defaultThick} ${Colors.background.primary}
          ...
          `}
              >
                ...
                <div className="flex items-center gap-2">
                  <Link
                    ...
                    target="_blank"
                    className={`
              transition ${Colors.border.specialThick} ${Colors.background.primary}
              ${Colors.hover.special} ${Colors.text.special}
            ...
            `}
                  ...
                  <button
                    onClick={() => handleRemoveSectionProblem(problem.id)}
                    className="
              p-2 rounded-md bg-red-500/10 border border-red-500/30
              text-red-400 hover:bg-red-500/20 hover:border-red-400/60
              transition cursor-pointer
            "
                  >
                    <Trash2 size={14} />
      ...
      <div className="mt-4 ml-1 flex gap-3">
        ...
        {activeTab == "PROBLEM" && (
          <button
            onClick={() => setIsBrowseProblemsOpen(true)}
            disabled={sectionProblems.length >= MAX_SECTION_PROBLEMS}
            className={`
              transition ${Colors.border.specialThick} ${Colors.background.secondary}
              ${Colors.hover.special} ${Colors.text.special}
              rounded-md px-3 py-1.5 text-xs cursor-pointer
      ...
      {/* --------------- Browse Problems Modal ---------------- */}
      <BrowseProblemsModal
        open={isBrowseProblemsOpen}
        onClose={() => setIsBrowseProblemsOpen(false)}
        sectionId={sectionId}
        attachedIds={sectionProblems.map((p) => p.id)}
        remainingSlots={MAX_SECTION_PROBLEMS - sectionProblems.length}
        onAdded={() => setProblemsRefetchKey((prev) => prev + 1)}
      />
    </div>
  );
};
