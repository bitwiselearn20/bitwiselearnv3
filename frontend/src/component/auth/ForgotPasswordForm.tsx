...
export default function ForgotPasswordForm({
    ...
    async function handleForgot(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            // Dev fallback: when SMTP isn't configured the backend returns the
            // OTP in the response so it can still be entered. Surface it here.
            const devOtp = res?.data?.otp;
            if (devOtp) {
                toast(`Email not configured — your OTP is: ${devOtp}`, {
                    duration: 60000,
                    icon: "🔑",
                });
            }
